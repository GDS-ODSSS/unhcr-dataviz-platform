...
#plot the chart
fig, ax = plt.subplots()
ax.stackplot(x, y, color = '#0072BC')

#set chart title
ax.set_title('Number of refugees | 1990-2021', loc='left', fontweight='bold', pad=15, fontsize=14, color='#000000')
...
```{python python_area_chart, echo=FALSE, out.width="100%", fig.height=5.75}
# import libraries
import numpy as np
import matplotlib.pyplot as plt
import pandas as pd
...
types=['refugees','asylum-seekers','idps','venezuelans_displaced_abroad','stateless_persons','others_of_concern']

#plot the chart
fig, ax = plt.subplots()

for i in types:
    temp = df[df['poc_type'] == i]
    plt.plot(temp.year, temp.poc_number)
    
plt.xlim(2015.5,2021.5)
plt.xticks([2016, 2021])
