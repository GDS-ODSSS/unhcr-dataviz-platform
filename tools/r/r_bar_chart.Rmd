---
title: "Bar chart with R"
title-prefix: "UNHCR  Dataviz"
author: "IPDA/GDS"
output:
  html_document:
    theme: null
    self_contained: false
    highlight: "pygments"
    includes:
      in_header: "../rmd_includes/in_head.html"
      before_body: "../rmd_includes/header.html"
      after_body:
        - "../rmd_includes/r_comparison.html"
        - "../rmd_includes/footer.html"
        - "../rmd_includes/footer_script.html"
params:
  tool: R
  chart_type: Bar chart
  url_cat: comparison
  url_chart: bar
---

```{r setup, include=FALSE}
knitr::opts_knit$set(base.dir = "../../assets/img/tools/",
                     base.url = "../../assets/img/tools/")
knitr::opts_chunk$set(echo = FALSE,
                      fig.path= paste0(tolower(params$tool),"/"),
                      message = FALSE,
                      warning = FALSE,
                      collapse = TRUE,
                      comment = "#>",
                      dev = "svglite",
                      fig.showtext = TRUE,
                      fig.retina = 2,
                      dpi = 300)
library(unhcrthemes)
library(tidyverse)
library(scales)
```

<main id="tools" class="container mt-5">

<!-- Breadcrumb is filled automatically with the text from Title and Tool parameter in YAML --> 
<nav aria-label="breadcrumb">
    <ol class="breadcrumb">
        <li class="breadcrumb-item"><a href="/">Home</a></li>
        <li class="breadcrumb-item"><a href="/tools/">Tools</a></li>
        <li class="breadcrumb-item"><a href="/tools/`r tolower(params$tool)`/">`r params$tool`</a></li>
        <li class="breadcrumb-item active" aria-current="page">`r rmarkdown::metadata$title`</li>
    </ol>
</nav>

<!-- Intro header --> 
<div class="my-5 text-center">

## `r rmarkdown::metadata$title`

<div class="row justify-content-center">
<div class="col-lg-8">

<!-- Only text to change below, the rest comes from the YAML --> 
In a column chart, each category is represented by a vertical rectangle, with the height of the rectangle being proportional to the values being plotted.

More about: <a href="/chart_gallery/`r params$url_cat`.html#`r params$url_chart`">`r params$chart_type`</a>

</div>
</div>
</div>


----

<!-- Beginning of section, should be preceded by hr (----) --> 
<section class="my-5">

### Bar chart

<div class="row">

<!-- First column for large screen --> 
<div class="col-lg-6">

```{r bar_code, echo=TRUE, eval=FALSE}
# Loading required packages
library(unhcrthemes)
library(tidyverse)
library(scales)

# Loading data
df_bar <- read_csv("https://raw.githubusercontent.com/GDS-ODSSS/unhcr-dataviz-platform/master/data/comparison/bar.csv")

ggplot(df_bar) +
  geom_col(aes(x = displaced_number, y = reorder(country_origin, displaced_number)),
           fill = unhcr_pal(n = 1, "pal_blue"),
           width = 0.8) +
  labs(title = "People displaced across borders by country of origin | 2021",
       x = "Number of people (in million)",
       caption = "Source: UNHCR Refugee Data Finder\n© UNHCR, The UN Refugee Agency") +
  scale_x_continuous(expand = expansion(c(0, 0.1)),
                     breaks = pretty_breaks(n = 7),
                     labels = label_number_si()) +
  theme_unhcr(grid = "X", axis = "y", axis_title = "x")
```

</div>
<!-- End First column --->

<!-- Second column for large screen --> 
<div class="col-lg-6">


```{r r_bar_chart, echo=FALSE, out.width="100%", fig.height=5.8}
# Loading data
df_bar <- read_csv("https://raw.githubusercontent.com/GDS-ODSSS/unhcr-dataviz-platform/master/data/comparison/bar.csv")

ggplot(df_bar) +
  geom_col(aes(x = displaced_number, y = reorder(country_origin, displaced_number)),
           fill = unhcr_pal(n = 1, "pal_blue"),
           width = 0.8) +
  labs(title = "People displaced across borders by country of origin | 2021",
       x = "Number of people (in million)",
       caption = "Source: UNHCR Refugee Data Finder\n© UNHCR, The UN Refugee Agency") +
  scale_x_continuous(expand = expansion(c(0, 0.1)),
                     breaks = pretty_breaks(n = 7),
                     labels = label_number_si()) +
  theme_unhcr(font_size = 13, grid = "X", axis = "y", axis_title = "x")
```

</div>
<!-- End Second column --->

</div>
</section>
<!-- End of section --->

----

<!-- Beginning of section, should be preceded by hr (----) --> 
<section class="my-5">

### Bar chart with labels

<div class="row">

<!-- First column for large screen --> 
<div class="col-lg-6">

```{r bar_labels_code, echo=TRUE, eval=FALSE}
# Loading required packages
library(unhcrthemes)
library(tidyverse)
library(scales)

# Loading data
df_bar <- read_csv("https://raw.githubusercontent.com/GDS-ODSSS/unhcr-dataviz-platform/master/data/comparison/bar.csv")

ggplot(df_bar) +
  geom_col(aes(x = displaced_number, y = reorder(country_origin, displaced_number)),
           fill = unhcr_pal(n = 1, "pal_blue"),
           width = 0.8) +
  geom_text(aes(x = displaced_number, y = reorder(country_origin, displaced_number),
                label = round(displaced_number / 1e6, 2)),
            hjust = -0.5, size = 8/.pt) +
  labs(title = "People displaced across borders by country of origin | 2021",
       subtitle = "Number of people (in million)",
       caption = "Source: UNHCR Refugee Data Finder\n© UNHCR, The UN Refugee Agency") +
  scale_x_continuous(expand = expansion(c(0, 0.1))) +
  theme_unhcr(grid = FALSE, axis = "y", axis_title = FALSE, axis_text = "y")
```

</div>
<!-- End First column --->

<!-- Second column for large screen --> 
<div class="col-lg-6">


```{r r_bar_labels_chart, echo=FALSE, out.width="100%", fig.height=5.8}
# Loading data
df_bar <- read_csv("https://raw.githubusercontent.com/GDS-ODSSS/unhcr-dataviz-platform/master/data/comparison/bar.csv")

ggplot(df_bar) +
  geom_col(aes(x = displaced_number, y = reorder(country_origin, displaced_number)),
           fill = unhcr_pal(n = 1, "pal_blue"),
           width = 0.8) +
  geom_text(aes(x = displaced_number, y = reorder(country_origin, displaced_number),
                label = round(displaced_number / 1e6, 2)),
            hjust = -0.5, size = 11.5/.pt) +
  labs(title = "People displaced across borders by country of origin | 2021",
       subtitle = "Number of people (in million)",
       caption = "Source: UNHCR Refugee Data Finder\n© UNHCR, The UN Refugee Agency") +
  scale_x_continuous(expand = expansion(c(0, 0.1))) +
  theme_unhcr(font_size = 13, grid = FALSE, axis = "y", axis_title = FALSE, axis_text = "y")
```

</div>
<!-- End Second column --->

</div>
</section>
<!-- End of section --->

<!-- To add sections just copy the whole code between the above section tag --->

</main>
